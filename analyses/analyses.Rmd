...
1. note that vahey is used to justify sample sizes, which are low in the literature.
2. use vahey's meta-analytic effect size to reproduce his power analyses and sample size recommendations. nope. 
3. use vahey's weighted average effect sizes presented in his forest plot, attempt to reproduce meta results using descriptions of his analytic method. nope.
4. use vahey's individual effect sizes presented in his supplementary materials, attempt to reproduce his weighted average effect sizes presented in his forest plot. nope.
5. go back to the original articles, reextract and convert their effect sizes, attempt to reproduce vahey's individual effect sizes presented in his supplementary materials. nope on two fronts: many errors, many absences. 
  - in considering his absences, note the inconsistent choices that were made that favored higher ES.
  - note problems in the selection strategy itself: unblinded, clinically relevant (violations), a priori predicable
6. conduct a new meta using a more complete list of correct ES and better analytic strategy.
7. make new power and sample size recommendations. compare the sample sizes employed in the published literature to this criterion: what % meet 80% power, what % reported sig results, what observed power these studies would have had using my meta estimate.
...
Reproducing Vahey's results involves numerous assumptions about details that are not explicated in the paper. For example, the Hunter & Schmidt method employed in the original meta-analysis weights the effect sizes by the inverse of the sample size for each study. However, (a) multiple sample sizes were obtained for several studies because they had mulitple effect sizes, and yet (b) the forest plot table reported in the manuscript reports integer values for all Ns that do not corrispond to these weighted Ns, despite weighted df (i.e., N-2) apparently being used to calculate the weighted mean effect sizes.  

In the absence of clear information about what the original authors did, the new meta analysis employed the weighted mean effect sizes, their CIs, and Ns reported in Vahey et al.'s forest plot, from which SEs were calculated.

[should i drop the code here re whether their weighted averaging can be replicated?]
...
data_for_meta_vahey <- 
  left_join(data_for_meta_vahey_from_supplementary_materials, 
            data_for_meta_vahey_from_forest_plot, 
            by = "article_abbreviated") %>%
