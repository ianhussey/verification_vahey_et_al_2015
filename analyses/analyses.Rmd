...
# Manuscript narrative outline

Section 1: the reproducibility of Vahey et al.'s (2015) data and results.

1. Vahey et al. is frequently cited to justify sample sizes in the IRAP literature. These sample sizes are very low (median sample size < 40).
2. I used Vahey et al.'s meta-analytic effect size to reproduce their power analyses and sample size recommendations. Computationally reproducible, but I found they made some weird choices, results are in the IRAP's favor.
3. I then used Vahey et al.'s weighted average effect sizes presented in their forest plot and descriptions of their analytic method to try to computationally reproduce their meta results. Fairly close, but can't reproduce.
4. Then I used Vahey et al.'s individual effect sizes reported in their supplementary materials to attempt to reproduce the weighted average effect sizes presented in their forest plot. Nope, several errors.
5. Then I went back to the original articles, reextracted and converted their effect sizes, and attempted to reproduce Vahey et al.'s individual effect sizes presented in their supplementary materials. Nope on multiple fronts: many errors, many absences, many questionable inclusions.

Section 2: a new meta-analysis using a complete and corrected set of effect sizes, and a more up to date analytic strategy.  

6. New meta. Effect size is less than half that reported in Vahey et al. (Vahey et al. *r* = .45 vs. new *r* = .18).
7. Finally, I conducted new power analyses using the new meta effect size. Recommended sample sizes were more than 8 times those reported in Vahey et al. (Vahey et al. Ns = 29 to 37 vs. new Ns = 240 to 782). To my knowledge, no or almost no studies in the published literature meet these sample size recommendations. 
