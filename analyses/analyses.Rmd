...
Vahey et al. (2015) do not mention applying any transformations to their data. However, they cited using code by Field & Gillett (2010) to implement their analyses. Inspection of this code reveals two transformations were applied: Overton's (1998, page 358) correction ($r_{adjusted} = r - (r*(1-r^2)) / (2*(n-3))$) and Fisher's r-to-z correction. This would explain why Vahey et al.'s (2015) confidence intervals are asymmetric in their forest plot.  

Reverse engineering the forest plot:

- effect sizes are weighted means, not transformed with either transformation
- their CIs however are likely the transformed CIs as they are asymmetrical - check using same method as ES?
- the meta ES is more likely to be the transformed ES without back-transformation. that would make the CIs congruent with Vahey et al, and the effect size only slightly different. It is hard to know.
  - Either the numerical result is approximately reproducible but Vahey mislabelled it, and therefore misconducted the power analyses; or the numerical result is not reproducible.
- either way, vahey does not explicitly state that they used fischers r to z transformations (although they make some reference to the method in plots), and they make no reference to an Overton transformation, although it is implemented in the code provided by Field & Gillett's (2010) which Vahey reported they used. This cannot be verfied directly as Vahey declined to share his code with me. 

Smaller issues:

- the forest plot employs the diamond shape to represent the CR intervals where it typically illustrates the CIs. 
- CR in the forest plot is labeled as CI, but elsewhere in text this numerical width is referred to as the CR.
...
data_vahey_forest_plot <- data_vahey %>%
  ...
  mutate(vi = ((ci_upper - ci_lower)/(1.96*2))^2)
...
metafor::forest(fit_reproduced,
                ...
                at = c(-.25, 0, .25, .5, .75, 1))
text(-.75, 17, "Reproduced from effect sizes in Vahey et al.'s forest plot", pos = 4)
text(1.25, 16.85, substitute(paste(italic('Z'), " [95% CI]")), pos = 2)
...
round(transf.ztor(fit_reproduced$b), 2)
